<div class="mailbox-content"><!-- MAIL BOX CONTENT DIV  -->
	<div class="table-responsive col-lg-12 col-md-12 col-xs-12 col-sm-12">
						...
						echo $obj_message->MJ_lawmgt_pagination($totlal_message,$p,$prev,$next,'page=message&tab=inbox');?>
		<table class="table">	
			<tbody>
				<tr> 			
					<th class="">
				...
				foreach($message as $msg)
				{
					...
					else
					{
						...
						<tr>				
							...
							<td>
								 <a href="?dashboard=user&page=message&tab=inbox&tab=view_message&from=inbox&id=<?php echo esc_attr($msg->message_id); ?>"> <?php echo wordwrap(esc_html($msg->msg_subject),10,"<br>\n",TRUE); /* if($obj_message->MJ_lawmgt_count_reply_item($msg->post_id)>=1){?><span class="badge badge-success pull-right"><?php echo esc_html($obj_message->MJ_lawmgt_count_reply_item($msg->post_id));?></span><?php } */ ?></a>
							</td>
							<td>
								<?php echo wordwrap(esc_html($msg->message_body),30,"<br>\n",TRUE);?>
							</td>
							<td>
								<?php  echo esc_html(MJ_lawmgt_getdate_in_input_box($msg->msg_date));?>
							</td>
						</tr>
					 <?php 
					}
