				...
				<tr>
					<td class="hidden-xs">
						...
						else
						{
							...
							echo esc_html(MJ_lawmgt_get_role_name_in_message($role[0]));
						}
						?></span>
					</td>
					<td><a href="?dashboard=user&page=message&tab=view_message&from=sendbox&id=<?php echo  esc_attr($msg_post->ID);?>"><?php echo wordwrap(esc_html($msg_post->post_title),10,"<br>\n",TRUE);?><?php if($obj_message->MJ_lawmgt_count_reply_item($msg_post->ID)>=1){?><span class="badge badge-success pull-right"><?php echo esc_html($obj_message->MJ_lawmgt_count_reply_item($msg_post->ID));?></span><?php } ?></a></td>
					 <td>						
						  <?php echo wordwrap(esc_html($msg_post->post_content),30,"<br>\n",TRUE); ?>
					</td>
				</tr>
				<?php 
				}
