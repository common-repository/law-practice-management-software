...
<script type="text/javascript">
	...
	{ 
		"use strict";
		vartime_entry = $('.time_entry_div').html();   	
	...
	<div class="panel-body"><!-- PANEL BODY DIV -->
		<form name="invoice_form" action="" method="post" class="form-horizontal" id="invoice_form" enctype='multipart/form-data'>	
			...
			<div class="form-group">
				...
				<div class="col-lg-4 col-md-4 col-sm-4 col-xs-12">
					<select class="form-control validate[required]" name="contact_name" id="invoice_contacts">	
						...
						{  		
							?>						
							<option value="<?php echo esc_attr($retrive_data->billing_contact_id);?>" <?php selected($retrive_data->billing_contact_id,$user_id) ?>><?php echo MJ_lawmgt_get_display_name(esc_html($retrive_data->billing_contact_id));?></option>						
							<?php 
						}		
						?>
					</select>
			...
			<div class="form-group">			
				...
				<div class="col-lg-4 col-md-4 col-sm-4 col-xs-12 has-feedback">
					...
					value="<?php if($edit){ echo MJ_lawmgt_getdate_in_input_box($invoice_info->generated_date);}elseif(isset($_POST['invoice_generated_date'])){ echo MJ_lawmgt_getdate_in_input_box($_POST['invoice_generated_date']); } ?>" readonly>
			...
			<div class="form-group">
				...
				<div class="col-lg-4 col-md-4 col-sm-4 col-xs-12 has-feedback">
					...
					value="<?php if($edit){ echo MJ_lawmgt_getdate_in_input_box($invoice_info->due_date);}elseif(isset($_POST['due_date'])){ echo MJ_lawmgt_getdate_in_input_box($_POST['due_date']); } ?>" readonly>
					<span class="fa fa-calendar form-control-feedback left" aria-hidden="true"></span>
					<span id="inputSuccess2Status2" class="sr-only">(success)</span>
				</div>	
				 
			</div>
			<?php wp_nonce_field( 'save_case_invoice_nonce' ); ?>
			<div class="header">	
		...
		<div class="time_entry_div">
						...
						<?php
							...
							{						
								...
								<div class="form-group">
									...
									<div class="col-sm-3 margin_bottom_5px">
										<input type="button" value="<?php esc_html_e('Delete','lawyer_mgt');?>" class=" height_39_px_css remove_time_entry btn btn-danger height_34_px_css">
									</div>
								</div><hr>
				 <?php
								}
			...
			<div class="offset-lg-2 col-lg-10 offset-md-2 col-md-10 offset-sm-2 col-sm-10 col-xs-12">
				<input type="button" value="<?php esc_attr_e('Add More Time Entries','lawyer_mgt') ?>" onclick="MJ_lawmgt_add_time_entry()" class="demo btn btn-success">
			</div>
	<!----- EXPENSES -------->
			<div class="header">	
							 ...
							 { 
								...
								<div class="form-group">
									...
									<div class="col-sm-3 margin_bottom_5px has-feedback">
										...
										<input  data-date-format="<?php echo MJ_lawmgt_bootstrap_datepicker_dateformat(get_option('lmgt_datepicker_format'));?>" class="date date_css form-control has-feedback-left validate[required]" type="text"  name="expense_date[]"  placeholder="<?php esc_html_e('Select Date','lawyer_mgt');?>" value="<?php if($edit){ echo esc_attr(MJ_lawmgt_getdate_in_input_box($data->expense_date));}elseif(isset($_POST['expense_date'])){ echo MJ_lawmgt_getdate_in_input_box($_POST['expense_date']); } ?>" readonly>
							...
							{						
							 ...
							 { 
								...
								<div class="form-group">
									...
									<div class="col-sm-3 margin_bottom_5px">
										<input type="button" value="<?php esc_html_e('Delete','lawyer_mgt');?>" class="remove_flat_entry btn btn-danger invoice_button">
									</div>
								</div><hr>
				 <?php
								}
