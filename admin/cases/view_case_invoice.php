				...
				<tbody>				
					<tr>
						...
						<?php 							
							...
							$address=esc_html($user_info->address);						
						...
						foreach($result_time as $data)
							{
								...
								if(!empty($tax))
								{
									...
									foreach($tax_explode as $tax1)
									{
										...
										$total_tax+= esc_attr($taxvalue->tax_value);
									} 
									if(!empty($total_tax))
									{	
					...
					<?php					
							...
							{							
								...
								if(!empty($tax))
								{
									...
									{
										$taxvalue=MJ_lawmgt_tax_value_by_id($tax1);
										$total_tax+= sanitize_text_field($taxvalue->tax_value);
									} 
									if(!empty($total_tax))
									{	
										$tax1_total_amount= sanitize_text_field($after_discount_flat_fee / 100 * $total_tax) ;
									}
								}
								 
								$flat_fee_total_tax+= sanitize_text_field($tax1_total_amount);
								$flat_fee_sub_total+= sanitize_text_field($flat_fee_sub);	
								$flat_entry_total_sub_amount= sanitize_text_field($flat_fee_sub - $discount + $tax1_total_amount);
								$flat_entry_total_amount+= sanitize_text_field($flat_entry_total_sub_amount);
								?>						 
								  <tr class="entry_list">
									<td class="align_center"><?php echo esc_html($id);?></td>
			...
			<table class="width_54" border="0">
				<tbody>
					<?php						
						...
						$subtotal_amount= sanitize_text_field($time_entry_sub_total+$expense_sub_total+$flat_fee_sub_total);
						 
						$discount_amount= sanitize_text_field($time_entry_discount+$expense_discount+$flat_fee_discount);
						 
						$tax_amount= sanitize_text_field($time_entry_total_tax+$expense_total_tax+$flat_fee_total_tax);
					 
						$due_amount= sanitize_text_field($subtotal_amount-$discount_amount+$tax_amount-$invoice_info->paid_amount);
						$paid_amount= sanitize_text_field($invoice_info->paid_amount);
						$grand_total= sanitize_text_field($subtotal_amount-$discount_amount+$tax_amount);
					?>
					<tr>
