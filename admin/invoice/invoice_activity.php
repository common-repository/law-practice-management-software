...
if(isset($_REQUEST['clear_log']))
{
	...
	if (file_exists($path)) 
	{
		...
		//Append data
		$file_contant_backup .= file_put_contents(LAWMS_Invoice_LOG_file_backup,$file_contant,FILE_APPEND) or die("Could not copy file");
		...
		<div class="panel-body"><!-- PANEL BODY DIV  -->
		    <form name="audit_log_form" action="" method="post" class="form-horizontal" id="audit_log_form">
				<div class="audit_button">
					<?php
						echo '<a href="'.LAWMS_PLUGIN_URL.'/download_log.php?mime=LAWMS_log.txt&title=audit_log.txt&token='.LAWMS_Invoice_LOG_file_download.'" class="btn btn-success">'.esc_html__('Download Log','lawyer_mgt').'</a>'; 
					?>
					<input type="hidden" name="path" value = "<?php echo LAWMS_Invoice_LOG_file;?>" >
					<input type="hidden" name="path_backup" value = "<?php echo LAWMS_Invoice_LOG_file_backup;?>" >
				...
				<div class="aduit_log_file">
					<table id="examlist" class="table table-striped" cellspacing="0" width="100%" align="center">
						<tbody>	
							...
							{
								foreach(array_reverse(file(LAWMS_Invoice_LOG_file)) as $line)
								{
									?>
									<tr>
