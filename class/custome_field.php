<?php 
class MJ_lawmgt_custome_field
{  /*<---START Lmgt_custome_field  CLASS--->*/
	/*<--- ADD CustomeField FUNCTION --->*/
	function MJ_lawmgt_add_custom_field($custome_data) 
	{			 
		global $wpdb;
		$wpnc_custom_fields = $wpdb->prefix . 'lmgt_custom_field';
		$wpnc_custom_field_dropdown_metas = $wpdb->prefix . 'lmgt_custom_field_dropdown_metas';		
		
		$custom_field_data['field_label']=sanitize_text_field($custome_data['field_label']);	
		...
		if(esc_attr($custome_data['action'])=='edit')	
		{ 	
			$custom_field_data['updated_by']=get_current_user_id();
			$custom_field_data['updated_at']=date("Y-m-d H:i:s");
			
			$whereid['id']= sanitize_text_field($custome_data['custom_field_id']);
			...
			{
				$d_label = sanitize_text_field($custome_data['d_label']);
			}
            else
			{
				$d_label =null;
			...
			if(!empty($d_label))
			{
				...
				foreach ($d_label as $key => $value)
				{
					...
					$custom_field_dropdown_data['custom_fields_id']=sanitize_text_field($custome_data['custom_field_id']);
					$custom_field_dropdown_data['option_label']=sanitize_text_field($label);
			...
			{
				$c_label = sanitize_text_field($custome_data['c_label']);
			}
            else
			{
				$c_label =null;
			...
			{
				//delete old value 
				$delete_custom_field_checkbox_data = $wpdb->query("DELETE FROM $wpnc_custom_field_dropdown_metas where custom_fields_id= ".sanitize_text_field($custome_data['custom_field_id']));
				
				foreach ($c_label as $key => $value)
				{
					$label = $c_label[$key];
					
					$custom_field_checkbox_data['custom_fields_id']=sanitize_text_field($custome_data['custom_field_id']);
					$custom_field_checkbox_data['option_label']=sanitize_text_field($label);
			...
			{
				$r_label = sanitize_text_field($custome_data['r_label']);
			}
            else
			{
				$r_label =null;
			...
			{
				//delete old value 
				$delete_custom_field_radio_data = $wpdb->query("DELETE FROM $wpnc_custom_field_dropdown_metas where custom_fields_id= ".sanitize_text_field($custome_data['custom_field_id']));
				
				foreach ($r_label as $key => $value)
				{
					$label = $r_label[$key];
					
					$custom_field_radio_data['custom_fields_id']=sanitize_text_field($custome_data['custom_field_id']);
					$custom_field_radio_data['option_label']=sanitize_text_field($label);
	...
	function MJ_lawmgt_get_all_custom_field_data()
	{
		global $wpdb;
		$wpnc_custom_fields = $wpdb->prefix . 'lmgt_custom_field';
	...
	function MJ_lawmgt_get_single_custom_field_data($custom_fields_id)
	{
		global $wpdb;
		$wpnc_custom_fields = $wpdb->prefix . 'lmgt_custom_field';
	...
	function MJ_lawmgt_get_single_custom_field__dropdown_meta_data($custom_field_id)
	{
		global $wpdb;
		$wpnc_custom_field_dropdown_metas = $wpdb->prefix . 'lmgt_custom_field_dropdown_metas';
	...
	public function MJ_lawmgt_delete_custome_field($id)
	{
		global $wpdb;
		$wpnc_custom_fields = $wpdb->prefix . 'lmgt_custom_field';
	...
	public function MJ_lawmgt_delete_selected_custome_field($record_id)
	{	
	...
	function MJ_lawmgt_getCustomFieldByModule($module) 
	{		
		global $wpdb;
		$wpnc_custom_fields = $wpdb->prefix . 'lmgt_custom_field';
	...
	function MJ_lawmgt_get_single_custom_field_meta_value($module,$module_record_id,$custom_field_id) 
	{		
		global $wpdb;
	...
	function MJ_lawmgt_getDropDownValue($custom_field_id) 
	{		
		global $wpdb;
		$wpnc_custom_field_dropdown_metas = $wpdb->prefix . 'lmgt_custom_field_dropdown_metas';
	...
	function MJ_lawmgt_getCheckedCheckbox($custom_field_id)
	{
		global $wpdb;
		$wpnc_custom_fields = $wpdb->prefix . 'lmgt_custom_field';
	...
	function MJ_lawmgt_add_custom_field_metas($module,$custom,$module_record_id) 
	{		
		global $wpdb;
		$wpnc_custom_field_metas = $wpdb->prefix . 'lmgt_custom_field_metas';
		...
		if(!empty($custom))
		{
			...
			{				
				$value = $custom[$key];
				$checkboxreturn =$this->MJ_lawmgt_getCheckedCheckbox($key);
	...
	function MJ_lawmgt_check_field_old_or_new($module,$module_record_id,$custom_field_id)
	{
		global $wpdb;
		$wpnc_custom_field_metas = $wpdb->prefix . 'lmgt_custom_field_metas';
	...
	function MJ_lawmgt_update_custom_field_metas($module,$custom,$module_record_id) 
	{		
		global $wpdb;
		...
		if(!empty($custom))
		{
			foreach($custom as $key=>$value)
			{
				$value = sanitize_text_field($custom[$key]);
				$checkboxreturn =$this->MJ_lawmgt_getCheckedCheckbox($key);
				
				$check_field_old_or_new=$this->MJ_lawmgt_check_field_old_or_new($module,$module_record_id,$key);
